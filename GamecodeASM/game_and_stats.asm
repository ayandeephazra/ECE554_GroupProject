...
llb R1, 0x0f                    # mmap address for clearing screen
lhb R1, 0xc0
...
llb R12, 0x00                   # counter for PLACE_SHIP_COUNT
lhb R12, 0x40
...
RUN:
    ...
    # If KEY[1] pressed:   
    llb R13, 0xF0	            # Check value for if one of the keys is pressed 
    lhb R13, 0xF0        
...
MOVING_DOWN:
    ...
    llb R12, 0x00               # counter for MOVING_DOWN_COUNT
    lhb R12, 0x40
...
UP:
    # check top of screen
    SUBI R4, R3, 0              # if R4 == 0, then at top of screen
    b neq, MOVING_UP            # can still move up
    b eq, RIGHT_TO_LEFT         # can no longer move up -> go to moving the meteor
...
MOVING_UP:
    ...
    llb R12, 0x00               # counter for MOVING_UP_COUNT
    lhb R12, 0x40
...
RIGHT_TO_LEFT:
	...
	llb R6, 0x04                # very left side of the screen
	lhb R6, 0xFC 

	subi R7, R7, 1			    # move img right by 1 unit
	sw R7, R1, 0			    # write XLOC

	sw R9, R1, 1                # write YLOC

	llb R4, 0x5			        # 3 => spaceship, asteroid => 5, blackout = 7
	sw R4, R1, 2			    # cntrl <= 5

	llb R12, 0x00               # counter for RIGHT_TO_LEFT_COUNT
	lhb R12, 0x40			
...
REMOVE_IMAGE:
	...
	llb R12, 0x00	            # counter for REMOVE_IMAGE_COUNT
	lhb R12, 0x40
...
LANE1:
    ...
    llb R12, 0x00               # counter for LANE1_COUNT
    lhb R12, 0x40
    LANE1_COUNT:
    subi R12, R12, 1
    b neq, LANE1_COUNT
...
LANE2:
    ...
    llb R9, 0x5A                # YLOC <= 10'h005A (right side of the screen)
    lhb R9, 0x00
    sw R9, R1, 1                
    ...
    llb R12, 0x00               # counter for LANE2_COUNT
    lhb R12, 0x40
...
LANE3:
    ...
    llb R9, 0xAA                # YLOC <= 10'h00AA (right side of the screen)
    lhb R9, 0x00
    sw R9, R1, 1                
    ...
    llb R12, 0x00               # counter for LANE3_COUNT
    lhb R12, 0x40
...
LANE4:
    ...
    llb R9, 0xFA                # YLOC <= 10'h00FA (right side of the screen)
    lhb R9, 0x00
    sw R9, R1, 1                
    ...
    LANE4_COUNT:                # counter for LANE4_COUNT
    subi R12, R12, 1
    b neq, LANE4_COUNT
...
LANE5:
    ...
    llb R9, 0x4A                # YLOC <= 10'h014A (right side of the screen)
    lhb R9, 0x01
    sw R9, R1, 1                
    ...
    llb R12, 0x00               # counter for LANE5_COUNT
    lhb R12, 0x40
...
LANE6:
    ...
    llb R9, 0x9A                # YLOC <= 10'h019A (right side of the screen)
    lhb R9, 0x01
    sw R9, R1, 1
    ...
    llb R12, 0x00               # counter for LANE6_COUNT
    lhb R12, 0x40
